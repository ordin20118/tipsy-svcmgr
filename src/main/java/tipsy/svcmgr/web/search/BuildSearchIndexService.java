...
import tipsy.es.EsEquipmentForIntegVo;
import tipsy.es.EsEquipmentVo;
import tipsy.es.EsIngredientForIntegVo;
import tipsy.es.EsIngredientVo;
import tipsy.es.EsRawLiquorForIntegVo;
...
public class BuildSearchIndexService {
	...
	public synchronized void buildRawLiquorInfoForIntegration(int tid, List<Integer> liquorIds) {
				...
				public void handleResult(ResultContext context) {
					...
					try {
						...
						if(true) {	// null data check
							...
							EsRawLiquorForIntegVo esLiquorInfo = new EsRawLiquorForIntegVo();
						...
						if(list.size() >= 200) {
							ESManagerForBatch.getInstance().createOrUpdateBulk(ESManagerForLiquorSearch.INDEX_INTEGRATION, list, null);	
							list.clear();
							log.debug("["+tid+"] update raw_liquor info ["+rowCount[0]+"]");							
						}
	...
	public synchronized void buildIngredientForIntegration(int tid, List<Integer> ingdIds) {
				...
				public void handleResult(ResultContext context) {
					...
					try {
						...
						if(true) {	// null data check
							...
							EsIngredientForIntegVo esIngdInfo = new EsIngredientForIntegVo();
	...
	public synchronized void buildEquipmentForIntegration(int tid, List<Integer> equipIds) {
				...
				public void handleResult(ResultContext context) {
					...
					try {
						...
						if(true) {	// null data check
							...
							EsEquipmentForIntegVo esEquipInfo = new EsEquipmentForIntegVo();
